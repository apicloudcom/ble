...
public class BleManager {
	 ...
	 */
		...
		private LeScanCallback mLeScanCallback = new LeScanCallback() {
			...
			public void onLeScan(BluetoothDevice device, int rssi, byte[] scanRecord) {
				...
				L.i("名字", device.getName()+""+"getAddress"+device.getAddress()+"device.getUuids();"+device.getUuids());
		...
		public void connect(UZModuleContext moduleContext,final Context mContext){
		    ...
		    try
		    {
		      ...
		      ((Activity)mContext).runOnUiThread(new Runnable()
		      {
		        public void run()
		        {
		          bluetoothGatt = device.connectGatt(mContext, false, 
		...
		private BluetoothGattCallback mBluetoothGattCallback = new BluetoothGattCallback()
		  {
		    public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState)
		    {
		      ...
		      else if (newState == 0)
		      {
		        ...
		        mConnectCallBackMap.remove(address);
		        connectCallBack(moduleContext, false, -1, "newState:" + 
		          newState);
		      }
		    }
		    ...
		    public void onServicesDiscovered(BluetoothGatt gatt, int status)
		    {
		      ...
		      else
		      {
		        String address = gatt.getDevice().getAddress();
		       discoverServiceCallBack(
		          mDiscoverServiceCallBackMap.get(address), null, false, 
		          status);
		      }
		    }
		...
		public void isConnected(UZModuleContext moduleContext){
			...
			isConnectedCallBack(moduleContext,isConnected,
					address);
		}
		/***
		 * 发现服务;
	...
	private void connectCallBack(UZModuleContext moduleContext, boolean status,
			int errCode, String detailErrorCode) {
		JSONObject ret = new JSONObject();

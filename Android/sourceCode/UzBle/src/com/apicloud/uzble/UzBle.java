...
public class UzBle extends UZModule {
	...
	public void jsmethod_initManager(UZModuleContext moduleContext) {
		...
		if (!single) {
			...
			if (!BleUtil.isBlePermission(this.context())) {
				initCallBack(moduleContext, "unauthorized");
			} else if (!BleUtil.isBleSupported(this.context())) {
...
//	打开蓝牙
	...
	public void jsmethod_scan(UZModuleContext moduleContext) {
		...
		boolean cleanBoolean = moduleContext.optBoolean("clean",true);
	 ...
	 * 清空扫描到设备的集合
	 */
	public void  jsmethod_clean(UZModuleContext moduleContext){
		if (!single) {
	...
	public void jsmethod_connect(UZModuleContext moduleContext) {
		if (!single) {
			...
			L.i("connectsingle");
			BleManager.getInstance().connect(moduleContext, this.context());
		}
	}
	...
	public void jsmethod_connectPeripherals(UZModuleContext moduleContext) {
		if (!single) {
			...
			BleManager.getInstance().connectPeripherals(moduleContext, this.context());
		}
	}
	...
	public void jsmethod_isConnected(UZModuleContext moduleContext) {
		if (!single) {
			String address = moduleContext.optString("peripheralUUID");
			if (this.mIBle != null) {
				isConnectedCallBack(moduleContext, this.mIBle.isConnected(address), address);
			} else {
	...
	public void jsmethod_discoverDescriptorsForCharacteristic(UZModuleContext moduleContext) {
		if (!single) {
			if (this.mIBle != null) {
				...
				this.mIBle.discoverDescriptorsForCharacteristic(moduleContext, address, serviceUUID, characteristicUUID);
			} else {
	...
	public void jsmethod_setNotify(UZModuleContext moduleContext) {
		if (!single) {
			if (this.mIBle != null) {
				...
				this.mIBle.setNotify(moduleContext, address, serviceUUID, characteristicUUID);
			} else {
				errcodeCallBack(moduleContext, 6);
			}
		} else {
	...
	public void jsmethod_setSimpleNotify(UZModuleContext moduleContext) {
		if (!single) {
			if (this.mIBle != null) {
				...
				this.mIBle.setSimpleNotify(moduleContext, address, serviceUUID, characteristicUUID);
			} else {
				errcodeCallBack(moduleContext, 6);
			}
		} else {
	...
	public void jsmethod_readValueForCharacteristic(UZModuleContext moduleContext) {
		if (!single) {
			if (this.mIBle != null) {
				...
				this.mIBle.readValueForCharacteristic(moduleContext, address, serviceUUID, characteristicUUID);
			} else {
				errcodeCallBack(moduleContext, 6);
			}
		} else {
	...
	public void jsmethod_readValueForDescriptor(UZModuleContext moduleContext) {
		if (!single) {
			if (this.mIBle != null) {
				...
				this.mIBle.readValueForDescriptor(moduleContext, address, serviceUUID, characteristicUUID, descriptorUUID);
			} else {
	...
	public void jsmethod_writeValueForCharacteristic(UZModuleContext moduleContext) {
		if (!single) {
			if (this.mIBle != null) {
				...
				this.mIBle.writeValueForCharacteristic(moduleContext, address, serviceUUID, characteristicUUID, value, intWriteType);
			} else {
	...
	public void jsmethod_writeValueForDescriptor(UZModuleContext moduleContext) {
		if (!single) {
			if (this.mIBle != null) {
				...
				this.mIBle.writeValueForDescriptor(moduleContext, address, serviceUUID, characteristicUUID, descriptorUUID, value);
			} else {
	...
	private void initIBle() {
		BleUtil.BLESDK sdk = BleUtil.getBleSDK(this.context());
		if (this.mIBle == null) {
			if (sdk == BleUtil.BLESDK.ANDROID) {
				this.mIBle = new AndroidBle(this.context());
			} else if (sdk == BleUtil.BLESDK.SAMSUNG) {
				this.mIBle = new SamsungBle(this.context());
			} else if (sdk == BleUtil.BLESDK.BROADCOM) {
				this.mIBle = new BroadcomBle(this.context());
			} else {
			}
		}
	}
	...
	public void getPeripheralCallBack(UZModuleContext moduleContext, Map<String, BleDeviceInfo> deviceMap) {
		...
		try {
			...
			Log.i("debug", "peripherals: " + peripherals.toString());
		} catch (JSONException e) {
			e.printStackTrace();
		}
	}
